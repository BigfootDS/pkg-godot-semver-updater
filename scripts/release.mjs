...
function getLastReleaseTag() {
  ...
  const tag = tags[0];
  if (tag === undefined) {
    throw new Error("No stable vX.Y.Z release tag is reachable from main.");
  }
  return tag;
}
...
function createReleasePlan() {
  ...
  if (manifest.version !== lastVersion) {
    throw new Error(
      `package.json is ${manifest.version}, but the last release tag is ${lastTag}. Versions are managed by CD.`,
    );
  }
...
function main() {
  if (process.argv[2] !== "--prepare") {
    throw new Error("Usage: node scripts/release.mjs --prepare");
  }
