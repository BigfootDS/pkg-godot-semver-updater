...
test("calculates the highest release required by conventional commits", async () => {
  const { determineReleaseType } = await loadReleaseTools();
  ...
  assert.equal(
    determineReleaseType(["fix: repair parser\n\nBREAKING CHANGE: the previous API is removed"]),
    "major",
  );
...
test("updates package and lockfile versions together", async () => {
  ...
  const directory = await mkdtemp(join(tmpdir(), "godot-semver-release-"));
  const packagePath = join(directory, "package.json");
  const lockPath = join(directory, "package-lock.json");
  await writeFile(packagePath, '{"name":"example","version":"0.0.1"}\n');
  await writeFile(
    lockPath,
    '{"name":"example","version":"0.0.1","packages":{"":{"name":"example","version":"0.0.1"}}}\n',
  );
